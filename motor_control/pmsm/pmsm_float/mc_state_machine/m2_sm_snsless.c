...
#define M2_ALIGN_CURRENT_RAMP_INC (0.2F)  /* [A/s] Align current ramp of Iq */
#define M2_CURRENT_RAMP_INC (1.0F)  /* [A/s] Current ramp of Iq during startup */
#define M2_MERGE_DURATION (200) /* [ms] time of merge to low bandwidth control after spinning*/
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
...
GFLIB_RAMP_T_FLT sIqRampParams;
...
static void M2_StateInitFast(void)
{
    ...
    g_sM2Drive.sFocPMSM.sUDcBusFilter.fltFltBfrX[0] = (M1_U_DCB_UNDERVOLTAGE / 2.0F) + (M1_U_DCB_OVERVOLTAGE / 2.0F);
    g_sM2Drive.sFocPMSM.sUDcBusFilter.fltFltBfrY[0] = (M1_U_DCB_UNDERVOLTAGE / 2.0F) + (M1_U_DCB_OVERVOLTAGE / 2.0F);
...
//    g_sM2Drive.sStartUp.sSpeedRampOpenLoopParams.fltRampDown = M2_OL_START_RAMP_INC;
    ...
    /* fault thresholds */
    g_sM2Drive.sFaultThresholds.fltUDcBusOver     = M1_U_DCB_OVERVOLTAGE;
    g_sM2Drive.sFaultThresholds.fltUDcBusUnder    = M1_U_DCB_UNDERVOLTAGE;
    g_sM2Drive.sFaultThresholds.fltUDcBusTrip     = M1_U_DCB_TRIP;
    ...
    g_sM2Drive.sStartUp.sCurrentRampOpenLoopParams.fltRampUp   = M2_ALIGN_CURRENT_RAMP_INC*M2_FOC_FREQ_VS_PWM_FREQ/M2_PWM_FREQ;
    g_sM2Drive.sStartUp.sCurrentRampOpenLoopParams.fltRampDown = M2_ALIGN_CURRENT_RAMP_INC*M2_FOC_FREQ_VS_PWM_FREQ/M2_PWM_FREQ;
    ...
    Flux_Init(&Flux_Obs);
    sIqRampParams.fltRampDown = M2_CURRENT_RAMP_INC*M2_FOC_FREQ_VS_PWM_FREQ/M2_PWM_FREQ;
    sIqRampParams.fltRampUp = M2_CURRENT_RAMP_INC*M2_FOC_FREQ_VS_PWM_FREQ/M2_PWM_FREQ;
    
    sParam.fltMin=0.0F;
    sParam.fltMax=21.0F;
    sParam.u16TableSize=22;
...
static void M2_StateStopFast(void)
{
    ...
    if ((g_bM2SwitchAppOnOff != FALSE) && (g_sM2Drive.eControl == kControlMode_Scalar || (g_sM2Drive.sSpeed.fltSpeedCmd != 0.0F) || 
       (g_sM2Drive.eControl == kControlMode_CurrentFOC && (g_sM2Drive.sMCATctrl.sIDQReqMCAT.fltQ != 0.0F))))
    {
        /* Set the switch on */
...
static void M2_StateRunReadyFast(void)
{
    ...
    switch (g_sM2Drive.eControl)
    {
        ...
        case kControlMode_CurrentFOC:
            if ((g_sM2Drive.sMCATctrl.sIDQReqMCAT.fltQ != 0.0F)||(g_sM2Drive.sMCATctrl.sIDQReqMCAT.fltD != 0.0F))
            {
...
//#ifdef M2_DEADTIME_TEST
                g_eM2StateRun = kRunState_Spin;
                M2_MCDRV_PWM3PH_EN(&g_sM2Pwm3ph);
                sIqRampParams.fltState = 0.0;
//#else
//                /* Transition to the RUN ALIGN sub-state */
//                M2_TransRunReadyAlign();
//#endif
...
static void M2_StateRunAlignFast(void)
{
    ...
    g_sM2Drive.sFocPMSM.sIDQReq.fltQ = GFLIB_Ramp_FLT(M2_OL_START_I, &g_sM2Drive.sStartUp.sCurrentRampOpenLoopParams);
    ...
    M2_PMSMFocCtrl(&g_sM2Drive.sFocPMSM);
}
...
static void M2_StateRunStartupFast(void)
{
    ...
    switch (g_sM2Drive.eControl)
    {
        ...
        default:
            ...
            {
                g_sM2Drive.sFocPMSM.sIDQReq.fltQ = GFLIB_Ramp_FLT(g_sM2Drive.sStartUp.fltCurrentStartup, &sIqRampParams);
            }
            else
            {
            	g_sM2Drive.sFocPMSM.sIDQReq.fltQ = MLIB_Neg_FLT(GFLIB_Ramp_FLT(g_sM2Drive.sStartUp.fltCurrentStartup, &sIqRampParams));
            }
    ...
    {
      g_sM2Drive.ui16CounterState--;
    }
    /* Switch to close loop  */
    if((MLIB_Abs_FLT(g_sM2Drive.sSpeed.fltSpeedFilt) > g_sM2Drive.sStartUp.fltSpeedCatchUp)
       && (g_sM2Drive.ui16CounterState <= 0U) 
       && (g_sM2Drive.sSpeed.fltSpeedFilt * g_sM2Drive.sSpeed.fltSpeedCmd) > 0.0F)
    {
        M2_TransRunStartupSpin();
    }
}
 ...
 * @return None
 */
int test_index,testCnt;
bool_t bDTtest;
float_t fltUdReq[25],fltUdSum,fltUdComp[25];
float_t sign=1.0;
static void M2_StateRunSpinFast(void)
{
    ...
    switch (g_sM2Drive.eControl)
    {
        ...
        case kControlMode_CurrentFOC:
            ...
            }else
            {
              if(testCnt>2000)
              {
                fltUdSum+=g_sM2Drive.sFocPMSM.sUDQReq.fltD;
              }
              if(testCnt++>=4000)
              {
                testCnt=0;
                if(test_index==0)
                {
                  g_sM2Drive.sFocPMSM.sIDQReq.fltD = 0.01F*sign;
                }
                g_sM2Drive.sFocPMSM.sIDQReq.fltQ = 0;
                g_sM2Drive.sFocPMSM.sIDQReq.fltD = g_sM2Drive.sFocPMSM.sIDQReq.fltD*1.2160418F;
                fltUdReq[test_index] = fltUdSum/2000;
                fltUdComp[test_index] = fltUdReq[test_index]-g_sM2Drive.sFocPMSM.sIDQReq.fltD/1.2160418F*54.3F;
                test_index++;
                fltUdSum=0;
              }
              if(test_index>21)
              {
                ...
                test_index = 0;
                testCnt = 0;
                g_sM2Drive.sFocPMSM.sIDQReq.fltD = 0;
              }
            }
#else 
            /* Current FOC */
              g_sM2Drive.sFocPMSM.sIDQReq.fltQ = GFLIB_Ramp_FLT(g_sM2Drive.sMCATctrl.sIDQReqMCAT.fltQ, &sIqRampParams);
              g_sM2Drive.sFocPMSM.sIDQReq.fltD = g_sM2Drive.sMCATctrl.sIDQReqMCAT.fltD;
#endif
...
static void M2_TransRunReadyAlign(void)
{
    ...
    /* set required alignment current to Iq */
    g_sM2Drive.sFocPMSM.sIDQReq.fltD = 0.0F;
    g_sM2Drive.sFocPMSM.sIDQReq.fltQ = 0.0F;
    g_sM2Drive.sStartUp.sCurrentRampOpenLoopParams.fltState = 0.0F;
...
static void M2_TransRunAlignStartup(void)
{
    ...
    {
    /* Disable Open loop mode in FOC module */
      g_sM2Drive.sFocPMSM.bPosExtOn = FALSE;
    }
    ...
    M2_MCDRV_PWM3PH_EN(&g_sM2Pwm3ph);
    sIqRampParams.fltState = 0.0F;
    g_sM2Drive.ui16CounterState = M2_MERGE_DURATION*M2_PWM_FREQ/M2_FOC_FREQ_VS_PWM_FREQ/1000;//0.2s for converge
...
//    if(g_sM2Drive.sSpeed.fltSpeedCmd > 0.0F)
//    {
//      g_sM2Drive.sSpeed.sSpeedRampParams.fltState = g_sM2Drive.sStartUp.fltSpeedCatchUp;
//    }else
//    {
//      g_sM2Drive.sSpeed.sSpeedRampParams.fltState = -g_sM2Drive.sStartUp.fltSpeedCatchUp;
//    }
...
void Flux_Init(Flux_Obs_t *ptr)
{
      ptr->R_Origin = M2_R;//54.3F;//[ohm], Rs
      ptr->L_Origin = (M2_LD+M2_LQ)/2;//0.236F;//[Henry], (Lq+Lq)/2
      ptr->Lambda_Origin = M2_FLUX;//0.18F;//[v/rad*s], Ke
      ptr->L_diff = M2_LQ-M2_LD;//0.036F;//(Lq-Ld)
      ptr->Gamma = M2_FLUX_GAMMA;//0.5;

      ptr->dt = 1.0F*M2_FOC_FREQ_VS_PWM_FREQ/M2_PWM_FREQ;
      ptr->sTo.f16PGain  = M2_FLUX_TO_KP_GAIN;
      ptr->sTo.i16PGainSh = M2_FLUX_TO_KP_SHIFT;
      ptr->sTo.f16IGain  = M2_FLUX_TO_KI_GAIN;
      ptr->sTo.i16IGainSh = M2_FLUX_TO_KI_SHIFT;
      ptr->sTo.f16ThGain  = M2_FLUX_TO_THETA_GAIN;
      ptr->sTo.i16ThGainSh = M2_FLUX_TO_THETA_SHIFT;
}
