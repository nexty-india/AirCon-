...
/*! @brief mcs field weakening structure */
typedef struct
{
    GDFLIB_FILTER_IIR1_T_FLT	sFwErrorFilter;		/* Field weakening error filter */				
    float_t			fltFwError;			/* Raw field weakening error */				
    float_t			fltFwErrorFilt;		/* Filtered field weakening error */	   
    float_t			fltUFwError;		/* Voltage error */
    float_t			fltIFwError;		/* Current error */				
                            
    float_t			fltSpeedFwOn;		/* Speed when field weakening is turned on */
    frac16_t			f16ILimit;			/* Max. allowable -D current (output from the PI controller) */
    
    GFLIB_CTRL_PI_P_AW_T_FLT    sFwPiParams;		/* FW PI controller parameters */
    bool_t 			bFwPiSatFlag;		/* Field weakening PI controller saturation flag */
} mcs_pmsm_fw_t;
...
typedef struct _mcs_pmsm_foc_a1
{
    ...
    GMCLIB_2COOR_ALBE_T_F16 sIAlBeFrac;       /* Alpha/Beta current (FRAC) */
    GMCLIB_2COOR_DQ_T_FLT sIDQ;               /* DQ current */
    GMCLIB_2COOR_DQ_T_FLT sIDQReq;            /* DQ required current */
    GMCLIB_2COOR_DQ_T_F16 sIDQReqFrac;            /* for debug */
