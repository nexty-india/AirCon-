...
uint32_t DCBus_ADC_count = 0;
uint32_t DC_ADC_count = 0;
//struct UIn_current =0;
GMCLIB_3COOR_T_FLT Comp_I;
uint32_t AC_Voltage_count = 0;
bool g_clear_pfc_oc_fault = false;
bool start_pfc_manual = false;
...
bool g_my_pfc_fault_clear = false;
void main(void)
{
	...
	while (1)
	{
		...
		{
			if(user_delay(7500,&relay_Delay))
			{
				RELAY_ON();
				Relay_startup_check = false;

			}
         }

		if(user_delay(12500,&pfc_Delay) && PFC_startup_check)
		{
				start_pfc_manual = true;
		...
		if(user_delay(52500,&Torq_compensation_Delay) && Torq_comp_check)
				{
			      // sTLObser.w16EnableFeedForwCompFlag = 1;
			       sTLObser.w16EnableFeedForwCompFlag = 1;
		     	   Torq_comp_check = false;
				}

		if(start_pfc_manual == true)

		{
			start_pfc_manual = false;
			pfc_flag = true;
			gsPFC_Drive.sFlag.RelayFlag = 1;
			gsPFC_Drive.ui16CounterState = DELAY_BEFORE_CURRENT_CTRL;
		}

		if(g_clear_pfc_oc_fault == true)
		{
					PFC_CLEAR_OVERCURRENT_FAULT();
		     		g_clear_pfc_oc_fault = false;
		}


//			 if(user_delay(10000,&store) == true)
//			 				{
...
void ADC0_IRQHandler(void)
{
	...
	DC_ADC_count = g_sM1Drive.sFocPMSM.fltUDcBus;
	Comp_I = g_sM1Drive.sFocPMSM.sIABC;
	AC_Voltage_count = gsPFC_Drive.sUInPeakDetection.fltUInMax;
	/* StateMachine call */
